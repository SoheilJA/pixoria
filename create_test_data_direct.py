#!/usr/bin/env python
"""
Direct script to create test article without using Django management command
...
def create_test_image(width=800, height=600, color="blue", filename="test.jpg"):
...
print(f"✓ Category: {category.title} ({'Created' if created else 'Exists'})")
...
for tag_title in tag_titles:
    ...
    tags.append(tag)
    print(f"✓ Tag: {tag.title} ({'Created' if created else 'Exists'})")
...
author, created = Author.objects.get_or_create(
    ...
    defaults={
        ...
        "avatar": create_test_image(200, 200, (100, 200, 100), "author_avatar.jpg"),
    },
)
print(f"✓ Author: {author.name} ({'Created' if created else 'Exists'})\n")
...
article, created = Article.objects.get_or_create(
    ...
    defaults={
        ...
        "category": category,
        "author": author,
        "cover_image": create_test_image(1600, 900, (75, 0, 130), "cover.jpg"),
...
print(f"✓ Reading Time: {article.reading_time} دقیقه")

# 5. Add Tags to Article
if created:
    article.tags.set(tags)
    print("✓ Tags added to article")

print("\n" + "=" * 60)
print("✅ Test article created successfully!")
print("=" * 60)
print(f"📄 Article Title: {article.title}")
print(f"🔗 Article URL: http://localhost:8000/blog/{article.slug}/")
print(f"✍️  Author: {article.author.name}")
print(f"📂 Category: {article.category.title}")
print(f'🏷️  Tags: {", ".join([tag.title for tag in tags])}')
print(f"📝 Content Length: {len(article.content)} characters")
print(f"⏱️  Estimated Reading Time: {article.reading_time} minutes")
print("=" * 60)
